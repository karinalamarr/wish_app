...
class WishManager(models.Model):
    def basic_validator(self, postData):
        ...
        if len(postData['wish']) < 3:
            errors['wish'] = "Your wish must contain more than 2 characters"
            return errors


class Wish(models.Model):
    wish = models.CharField(max_length=225)
    poster = models.ForeignKey(User, related_name="wish")
    user_who_wished = models.ManyToManyField(User, related_name="wish_came_true")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    objects = WishManager() 
