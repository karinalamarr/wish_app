...
def success (request):
    if ('user' in request.session):
        return render(request, 'wish_app.success.html')
    else:
        return redirect('/')
...
def add_wish(request):
    if request.method =='POST':
            kept = User.objects.get(name = request.session['user'])
            Wish.objects.create(wish=request.POST['wish'], poster=kept)
            print("You made your wish")
